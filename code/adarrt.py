...
class AdaRRT():
    ...
    def build(self):
        ...
        for k in range(self.max_iter):
            ...
            if np.random.rand() < 0.2:
                random_sample_point = self._get_random_sample_near_goal()
            else:
                random_sample_point = self._get_random_sample()
            nearest_neighbor = self._get_nearest_neighbor(random_sample_point)
            new_node = self._extend_sample(random_sample_point, nearest_neighbor)
    ...
    def _trace_path_from_start(self, node=None):
        ...
        # FILL in your code here
        path = []
        current_node = node or self.goal
